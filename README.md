...
### **Reward Logic (Partial Progress)**
The environment does not use sparse binary rewards. It uses a **Logarithmic SNR** scale.
$$R = \frac{\log_{10}(\text{SNR} + 1)}{\log_{10}(\text{Max\_Theoretical\_SNR})}$$
...
1. Start the physics server: `uvicorn server.app:app --host 0.0.0.0 --port 7860`
2. Export your keys: `export HF_TOKEN="your_key"`
3. Run the agent: `python inference.py`